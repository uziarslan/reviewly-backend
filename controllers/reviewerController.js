...
exports.getAllReviewers = async (req, res, next) => {
  try {
    ...
    // NOTE: staging-only — status filter intentionally removed so the client can
    // preview unpublished reviewers. Restore `status: "published"` before merging to main.
    const filter = { type: { $ne: "trial_assessment" } };
