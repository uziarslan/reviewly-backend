/**
 * Restore missing trial assessment reviewers.
 * Unlike seedTrialAssessment.js, this script does NOT delete existing trial
 * reviewers — it only inserts the ones that are missing (matched by slug).
 * Safe to run on production / on environments that already have attempts
 * referencing existing reviewer IDs.
 *
 * Run: npm run restore:trial
 */
...
async function restore() {
  ...
  for (const reviewer of TRIAL_REVIEWERS) {
    const existing = await Reviewer.findOne({ slug: reviewer.slug });
    if (existing) {
      console.log(`⏭   ${reviewer.slug} already exists  →  ${existing._id}`);
      skipped += 1;
    } else {
      const doc = await Reviewer.create(reviewer);
      console.log(`✅  ${reviewer.slug} created  →  ${doc._id}`);
      created += 1;
    }
  }

  console.log(`\n📋  Restore complete: ${created} created, ${skipped} skipped.`);
